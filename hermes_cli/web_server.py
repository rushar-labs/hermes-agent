...
def get_model_info(profile: Optional[str] = None):
    ...
    try:
        ...
        # Skip the metadata probe against a loopback provider endpoint when the
        # operator has already pinned model.context_length. A local bridge or
        # proxy often implements only the generation route (e.g. POST
        # /v1/messages); probing /models or another discovery route against it
        # fails and surfaces false provider-down/cooldown noise even though
        # generation is healthy. With an explicit override configured, the probe
        # cannot tell us anything the operator has not already stated.
        auto_ctx = 0
        if config_ctx_int > 0 and base_url:
            from urllib.parse import urlparse
            _host = (urlparse(base_url).hostname or "").lower()
            if _host in {"localhost", "127.0.0.1", "::1"}:
                auto_ctx = config_ctx_int
        if auto_ctx == 0:
            ...
            try:
                ...
                auto_ctx = get_model_context_length(
                    ...
                    base_url=base_url,
                    provider=provider,
                    config_context_length=None,  # ignore override; want auto value
                )
            except Exception:
                auto_ctx = 0
